...
TEXT = (
    "corriger le titre","changer le titre","modifier le titre","corriger une faute",
    "corriger une faute de frappe","corriger l'orthographe","faute de frappe",
    "changer le texte","modifier le texte","corriger le texte","modifier une phrase",
    "corriger une phrase","changer le libellé","modifier le libellé","corriger le message",
    "corriger un message","modifier le message","modifier un message","texte affiché",
    "formulation visible","présentation de la page","présentation uniquement",
    "simple changement de texte","corriger la page","modifier la page","changer la page",
    "corriger le mot","modifier le mot","changer le mot","modifier uniquement la présentation",
    "présentation de l'écran","texte à corriger",
)
MEDIUM = (
...
OBJECTS = (
    "architecture","backend","frontend","base de données","base","données","stockage",
    "authentification","auth","autorisation","sécurité","paiement","migration",
    "synchronisation","services essentiels","plusieurs services","plusieurs composants",
    "plusieurs couches","couches fondamentales","comptes utilisateurs","logique interne",
...
COMPLEX = (
    ...
    "remplacer le système central qui","refonte complète","refonte totale","refonte profonde",
    "refactorisation","refactor","auth à revoir","db à modifier","base de données à modifier",
    "backend à modifier","backend à revoir","architecture à revoir","architecture à modifier",
    "migration réelle","déploiement réel","déploiement en production",
)
DANGEROUS = (
    ...
    "directement en production","exécuter réellement","l'exécuter réellement",
...
IRREVERSIBLE = (
    ...
    "faire disparaître définitivement","supprimer définitivement",
    "remplacer définitivement","rendre la modification définitive",
)
UNCERTAIN = (
...
NEGATION = (
    "sans ","ne pas ","ne rien ","ne surtout pas ","surtout ne pas ",
    "aucun ","aucune ","pas de ","jamais ",
)
...
def _clauses(text):
    return [c.strip() for c in re.split(r"\s*(?:,|;|\bmais\b|\bpuis\b|\bet\b)\s*", text) if c.strip()]

def _negated(clause):
    return has_any(clause, NEGATION)

def _active(text, terms):
    return any(has_any(c, terms) and not _negated(c) for c in _clauses(text))

def _action(text):
    return _active(text, (
        "modifier","modifie","modifiez","changer","change","changez","corriger",
        "corrige","corrigez","réparer","repenser","revoir","réorganiser","remplacer",
        "effectuer","appliquer","déployer","supprimer","détruire","ajouter","refaire",
        "refondre","reconfigurer","transformer","adapter","migrer","connecter","rendre",
        "toucher","améliorer","tester","préparer","analyser","étudier","simuler",
        "diagnostiquer","résoudre","construire","examiner","vérifier","exécuter","lancer",
    ))
...
def classify(task: str):
    ...
    uncertain = has_any(text, UNCERTAIN)

    if has_any(text, DESCRIPTIVE) and not _action(text):
        level, debate, human = "simple", False, False
    else:
        negated = has_any(text, ("sans ","ne pas ","ne rien ","aucune ","aucun "))
        medium = has_any(text, MEDIUM)
        semantic = has_any(text, COMPLEX)
        dangerous = _active(text, DANGEROUS)
        technical = _active(text, OBJECTS)
        irreversible = has_any(text, IRREVERSIBLE) and _action(text)
        local_complex_test = (
            has_any(text, ("tester une ","tester le ","tester ","construire "))
            and has_any(text, OBJECTS)
            and has_any(text, ("sans déployer","sans la déployer","sans l'appliquer","sans appliquer"))
        )
        independent_positive = any(_action(c) and not _negated(c) for c in clauses)
        protected = negated and not independent_positive

        if local_complex_test:
            level = "complex"
        elif protected and not medium:
            level = "simple"
        elif semantic or dangerous or irreversible:
            level = "complex"
        elif medium:
            level = "medium"
        elif technical:
            level = "medium" if has_any(text, ("toucher au backend","améliorer le paiement")) else "complex"
        elif uncertain:
            level = "medium"
        elif has_any(text, TEXT):
            level = "medium" if compound else "simple"
        else:
            level = "medium" if compound else "simple"

        if compound and not dangerous and not irreversible and has_any(text, ("ne pas ","ne rien ","ne surtout pas ","sans ")):
            if not local_complex_test and not semantic:
                level = "medium" if medium else "simple"

        if compound and (semantic or dangerous or irreversible or local_complex_test or
                          (has_any(text, ("supprimer","données","comptes")) and _action(text))):
            level = "complex"

        debate = level == "complex" or uncertain
        human = False
        if dangerous or irreversible:
            human, debate = True, True
        if has_any(text, REAL) and _action(text) and not protected:
            human, debate = True, True
        if compound and has_any(text, ("supprimer","données","comptes")) and _action(text):
            human, debate = True, True
        if local_complex_test and not has_any(text, REAL):
            human, debate = False, True
        if compound and dangerous:
    ...
    return {"difficulty": difficulty,"risk": risk,"level": level,"agents": agents,
            "debate": debate,"arbitration": False,"human": human,"uncertainty": uncertain}
