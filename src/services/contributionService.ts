...
export async function getContributionsValidees(matiere?:string,niveau?:string,limitCount=10):Promise<Contribution[]>{try{let q:any=query(collection(db,'contributions'),where('statut','==','validé'));if(matiere)q=query(q,where('niveau','==',niveau));const snap=await getDocs(q);return snap.docs.map(d=>({id:d.id,...d.data()})).slice(0,limitCount) as Contribution[];}catch(e){console.error(e);return[];}}
