...
import { collection, addDoc, serverTimestamp, query, where, getDocs, doc, getDoc, updateDoc, deleteDoc, increment } from 'firebase/firestore';
...
export async function soumettreContribution(nouvelleContribution: NouvelleContribution): Promise<{ contributionId:string; statut:ContributionStatut }> {
  ...
  try {
    const response=await fetch(nouvelleContribution.fichierUri); if(!response.ok)throw new Error('Lecture fichier impossible');
    ...
    const contributionData={type:nouvelleContribution.type,titre:nouvelleContribution.titre,matiere:nouvelleContribution.matiere,niveau:nouvelleContribution.niveau,serie:nouvelleContribution.serie||null,description:nouvelleContribution.description||null,tags:nouvelleContribution.tags||[],prix:nouvelleContribution.prix||0,statut:'en_modération' as ContributionStatut,auteur:{userId:nouvelleContribution.userId,nom:'',role:'repetiteur'},fichier:{url:fileUrl,nom:safeName,taille:Math.round(size/1024),type:blob.type||''},date_soumission:serverTimestamp(),telechargements:0,revenus_generes:0,score_ia:0};
    const docRef=await addDoc(collection(db,'contributions'),contributionData);
    try{const evaluation=JSON.parse(await utiliserAgent('moderateur',`Évalue cette contribution: ${nouvelleContribution.titre}\nMatière: ${nouvelleContribution.matiere}\nNiveau: ${nouvelleContribution.niveau}`,{userId:nouvelleContribution.userId,userRole:'repetiteur',matiere:nouvelleContribution.matiere,niveau:nouvelleContribution.niveau}));await updateDoc(docRef,{score_ia:evaluation.score||0,commentaire_modération:evaluation.commentaire||''});}catch{await updateDoc(docRef,{score_ia:60});}
...
export async function getContributionsByUser(userId:string,statut?:ContributionStatut):Promise<Contribution[]>{try{let q:any=query(collection(db,'contributions'),where('auteur.userId','==',userId));if(statut)q=query(q,where('statut','==',statut));const snap=await getDocs(q);return snap.docs.map(d=>({id:d.id,...d.data()})) as Contribution[];}catch(e){console.error(e);return[];}}
export async function getContributionsEnModeration():Promise<Contribution[]>{try{const q=query(collection(db,'contributions'),where('statut','in',['en_modération','en_attente','modification_demandée']));const snap=await getDocs(q);return snap.docs.map(d=>({id:d.id,...d.data()})) as Contribution[];}catch(e){console.error(e);return[];}}
export async function getContributionsValidees(matiere?:string,niveau?:string,limitCount=10):Promise<Contribution[]>{try{let q:any=query(collection(db,'contributions'),where('statut','==','validé'));if(matiere)q=query(q,where('matiere','==',matiere));if(niveau)q=query(q,where('niveau','==',niveau));const snap=await getDocs(q);return snap.docs.map(d=>({id:d.id,...d.data()})).slice(0,limitCount) as Contribution[];}catch(e){console.error(e);return[];}}
