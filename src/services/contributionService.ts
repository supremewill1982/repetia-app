...
export async function soumettreContribution(nouvelleContribution: NouvelleContribution): Promise<{ contributionId: string; statut: ContributionStatut }> {
  ...
  try {
    ...
    const contributionData = {
      ...
      statut: 'en_attente' as ContributionStatut,
      auteur: { userId: nouvelleContribution.userId, nom: '', role: 'repetiteur' as const },
      fichier: { url: fileUrl, nom: safeName, taille: Math.round(size / 1024), type: blob.type || '' },
      date_soumission: serverTimestamp(), telechargements: 0, revenus_generes: 0, score_ia: 0,
    };
...
export async function getContributionsByUser(userId: string, statut?: ContributionStatut): Promise<Contribution[]> {
  try { let q: any = query(collection(db, 'contributions'), where('auteur.userId', '==', userId)); if (statut) q = query(q, where('statut', '==', statut)); const snap = await getDocs(q); return snap.docs.map(d => ({ id: d.id, ...d.data() })) as Contribution[]; } catch (e) { console.error(e); return []; }
}
export async function getContributionsEnModeration(): Promise<Contribution[]> {
  try { const q = query(collection(db, 'contributions'), where('statut', 'in', ['en_attente', 'en_modération', 'modification_demandée'])); const snap = await getDocs(q); return snap.docs.map(d => ({ id: d.id, ...d.data() })) as Contribution[]; } catch (e) { console.error(e); return []; }
}
export async function getContributionsValidees(matiere?: string, niveau?: string, limitCount = 10): Promise<Contribution[]> {
  try { let q: any = query(collection(db, 'contributions'), where('statut', '==', 'validé')); if (matiere) q = query(q, where('matiere', '==', matiere)); if (niveau) q = query(q, where('niveau', '==', niveau)); const snap = await getDocs(q); return snap.docs.map(d => ({ id: d.id, ...d.data() })).slice(0, limitCount) as Contribution[]; } catch (e) { console.error(e); return []; }
}
