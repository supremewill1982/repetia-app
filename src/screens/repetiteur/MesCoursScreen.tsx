...
export default function MesCoursScreen({ navigation }: any) {
  ...
  const ouvrirContribuer=()=>navigation.getParent()?.navigate('Contribuer');
  const ouvrirDetails=(id:string)=>navigation.getParent()?.navigate('Profil',{screen:'ContributionDetails',params:{contributionId:id}});
  const ouvrirEdition=(id:string)=>navigation.getParent()?.navigate('Profil',{screen:'EditContribution',params:{contributionId:id}});
