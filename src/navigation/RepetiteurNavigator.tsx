...
function RepetiteurStackNavigator() {
  ...
  return <Stack.Navigator initialRouteName="ProfilRepetiteur" screenOptions={{headerStyle:{backgroundColor:colors.surface,elevation:0,shadowOpacity:0},headerTintColor:colors.text,headerTitleStyle:{fontWeight:'bold',color:colors.text}}}>
...
export default function RepetiteurNavigator() {
  const { colors } = useTheme();
  return <Tab.Navigator initialRouteName="Profil" screenOptions={{
    tabBarActiveTintColor:colors.primary,
    tabBarInactiveTintColor:colors.textMuted,
    tabBarStyle:{backgroundColor:colors.surface,borderTopWidth:0,elevation:12,height:72,paddingTop:7,paddingBottom:12},
    tabBarLabelStyle:{fontSize:11,fontWeight:'600'},headerShown:false,
  }}>
    <Tab.Screen name="Profil" component={RepetiteurStackNavigator} options={{tabBarLabel:'Profil',tabBarIcon:({color,size})=><MaterialCommunityIcons name="account" size={size} color={color}/>}}/>
    <Tab.Screen name="Contribuer" component={ContribuerCoursScreen} options={{tabBarLabel:'Contribuer',tabBarIcon:({color,size})=><MaterialCommunityIcons name="file-document-edit" size={size} color={color}/>}}/>
    <Tab.Screen name="MesCours" component={MesCoursScreen} options={{tabBarLabel:'Mes cours',tabBarIcon:({color,size})=><MaterialCommunityIcons name="file-document-multiple" size={size} color={color}/>}}/>
    <Tab.Screen name="Paiement" component={DemanderPaiementScreen} options={{tabBarLabel:'Paiement',tabBarIcon:({color,size})=><MaterialCommunityIcons name="bank-transfer" size={size} color={color}/>}}/>
    <Tab.Screen name="Reservations" component={MesReservationsTuteurScreen} options={{tabBarLabel:'Réservations',tabBarIcon:({color,size})=><MaterialCommunityIcons name="calendar-clock" size={size} color={color}/>}}/>
  </Tab.Navigator>;
}
